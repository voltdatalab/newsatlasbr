#' Retrieve data on Brazilian news organizations by state
#'
...
#'
#' @usage organizations_state(uf, news = 1, media = "all")
#'
#' @param uf the abbreviation of the Brazilian state for which the user wants to
#'   retrieve data. User should choose only one state as inputs. "All" returns
#'   data for every news organization found by News Atlas. The user should note
#'   that \code{uf = "all"} takes longer to run.
...
#'   Atlas' internal state ID. \item uf: abbreviation of the state's name. \item
#'   region_id: News Atlas' internal region ID. \item regiao: name of the
...
#'   profile and others). }
#'
#' @details \code{organizations_state} returns a detailed dataset on Brazilian
...
#'
#' municipalities_with_media <- organizations_state(uf = "SP")
#'

organizations_state <- function(uf,
                                news = 1,
                                media = "all"){

  if(missing(uf)){

    stop("Please insert a valid state in the \"uf\" argument.")
  ...
  banco <- extract_data_atlasbr_api(paste0("https://api.atlas.jor.br/api/v1/data/analytic?estado=", uf))
